...
def save_current_order_id(order_id):
    with open('current_order_id.txt', 'w') as file:
        file.write(str(order_id))

def load_current_order_id():
    try:
        with open('current_order_id.txt', 'r') as file:
            return int(file.read())
    except FileNotFoundError:
        return 0
...
def main():
    try:
        ...
        order_id = load_current_order_id()
        for i in range(10):
        ...
        save_current_order_id(order_id)
